...
object MyProgram:
  ...
  def fib(n: Int): Int =
    @tailrec
    def inner(pp: Int, p: Int, i: Int): Int = if i == n then pp else inner(p, p + pp, i + 1)
    inner(0, 1, 0)
...
object PolymorphicFunctions:
  ...
  def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean =
    if as.length <= 1 then true else
      as.sliding(2).toList.forall { case Array(a, b) => a == b || gt(b, a) }
